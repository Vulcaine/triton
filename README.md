...
> Tiny C++ project manager that wires up **CMake + vcpkg** and keeps a clean, modular layout.
...
- Link specific components to those deps via `"link": ["depName", …]`
- Triton generates the right `find_package` / `add_subdirectory` / `target_link_libraries` lines into managed regions.
